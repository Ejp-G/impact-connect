...
// Chaque jeudi à 19h — dernière relance avant le dimanche suivant.
export async function GET(request) {
  ...
  for (const integratorId of integratorIds) {
    ...
    const { error: logError } = await supabase.from('push_notification_log').insert({
      user_id: integratorId, notif_type: PUSH_TYPES.JEUDI, week_start: sunday, contact_count: count,
    })
    if (logError) {
      if (logError.code !== '23505') console.error('push-jeudi log error', integratorId, logError)
      continue
    }

    const title = 'Dernière relance'
    const message = count === 1
      ? 'Il te reste 1 jeune à contacter parmi les nouveaux attribués dimanche. Pense à effectuer le contact avant dimanche.'
      : `Il te reste ${count} jeunes à contacter parmi les nouveaux attribués dimanche. Pense à effectuer les contacts avant dimanche.`

    await supabase.from('notifications').insert({
      user_id: integratorId, type: PUSH_TYPES.JEUDI, title, message, link: '/suivi',
    })
