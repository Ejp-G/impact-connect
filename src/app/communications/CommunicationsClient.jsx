...
import { MessageCircle, Smartphone, Mail, Pencil, Clock, FileText } from '@/lib/icons'

const CH_COLORS = { whatsapp:'#25D366', sms:'#0B3D91', email:'#F97316' }
const CH_ICON_MAP = { whatsapp: MessageCircle, sms: Smartphone, email: Mail }
...
export default function CommunicationsClient({ templates, logs, contacts }) {
  ...
  useRealtimeRefresh(['contacts', 'communication_logs'])
  ...
  const TABS = [
    ['compose', 'Composer', Pencil],
    ['logs', 'Historique', Clock],
    ['templates', 'Modèles', FileText],
  ]

  const ChannelIcon = CH_ICON_MAP[channel]

  return (
    <div style={{maxWidth:1100}}>
      <div style={{display:'flex',gap:8,marginBottom:24}}>
        {TABS.map(([t,l,Icon])=>(
          <div key={t} onClick={()=>setTab(t)} style={{padding:'8px 18px',borderRadius:10,cursor:'pointer',fontSize:13,fontWeight:600,background:tab===t?'var(--n)':'#F1F5F9',color:tab===t?'#fff':'var(--gd)',display:'flex',alignItems:'center',gap:7}}>
            <Icon size={14} strokeWidth={2} /> {l}
          </div>
        ))}
      </div>
      ...
      {tab === 'compose' && (
        <div className="g2r">
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div className="card">
              <div style={{fontSize:15,fontWeight:700,marginBottom:16}}>Nouveau message</div>
              ...
              <div className="form-group"><label className="form-label">Canal</label>
                <div style={{display:'flex',gap:8}}>
                  {[['whatsapp','WhatsApp'],['sms','SMS'],['email','Email']].map(([v,l])=>{
                    const Icon = CH_ICON_MAP[v]
                    return (
                      <div key={v} onClick={()=>setChannel(v)} style={{flex:1,padding:10,borderRadius:10,border:`2px solid ${channel===v?CH_COLORS[v]:'var(--br)'}`,background:channel===v?`${CH_COLORS[v]}15`:'#fff',textAlign:'center',fontSize:12,fontWeight:600,color:channel===v?CH_COLORS[v]:'#64748B',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                        <Icon size={14} strokeWidth={2} /> {l}
                      </div>
                    )
                  })}
              ...
              <button onClick={sendMessage} disabled={sending} className="btn btn-primary" style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                {sending ? 'Envoi…' : <><ChannelIcon size={14} strokeWidth={2} /> Envoyer via {channel}</>}
              </button>
            </div>
          </div>
          ...
          <div className="card" style={{height:'fit-content'}}>
            ...
            {templates.map(t=>{
              const Icon = CH_ICON_MAP[t.channel]
              return (
                <div key={t.id} onClick={()=>applyTemplate(t)} style={{padding:'12px',borderRadius:10,border:`1px solid ${selectedTemplate?.id===t.id?CH_COLORS[t.channel]:'var(--br)'}`,background:selectedTemplate?.id===t.id?`${CH_COLORS[t.channel]}10`:'#F8FAFC',cursor:'pointer',marginBottom:8}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                    ...
                    <span style={{fontSize:10,background:CH_COLORS[t.channel]+'20',color:CH_COLORS[t.channel],padding:'2px 6px',borderRadius:999,fontWeight:600,display:'flex',alignItems:'center',gap:4}}>
                      <Icon size={10} strokeWidth={2} /> {t.channel}
                    </span>
                  </div>
      ...
      {tab === 'logs' && (
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <table>
            ...
            <tbody>
              {logs.map(l=>{
                const Icon = CH_ICON_MAP[l.channel]
                return (
                  <tr key={l.id}>
                    <td style={{fontWeight:600,fontSize:13}}>{l.contact?.first_name} {l.contact?.last_name}</td>
                    <td><span style={{fontSize:11,background:CH_COLORS[l.channel]+'20',color:CH_COLORS[l.channel],padding:'2px 8px',borderRadius:999,fontWeight:600,display:'inline-flex',alignItems:'center',gap:4}}>
                      <Icon size={11} strokeWidth={2} /> {l.channel}
                    </span></td>
            ...
            return (
              <div key={t.id} className="card" style={{borderTop:`3px solid ${CH_COLORS[t.channel]}`}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                  ...
                  <span style={{fontSize:11,background:CH_COLORS[t.channel]+'20',color:CH_COLORS[t.channel],padding:'2px 8px',borderRadius:999,fontWeight:600,display:'flex',alignItems:'center',gap:4}}>
                    <Icon size={11} strokeWidth={2} /> {t.channel}
                  </span>
                </div>
