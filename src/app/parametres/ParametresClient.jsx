...
export default function ParametresClient({ settings, communes, profile }) {
  ...
  const Row = ({ label, desc, children }) => (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 0',borderBottom:'1px solid #F1F5F9',gap:12,flexWrap:'wrap'}}>
  ...
  /* ── VERSION MOBILE — accordeon plein ecran ── */
  if (isMobile) {
    return (
        ...
        {TABS.map(t => {
          ...
          return (
            <div key={t.id} className="acc-item">
              <div className="acc-head" onClick={()=>setOpenMobile(open ? null : t.id)}>
                <span>{t.label}</span>
                <span className="acc-chev">{open ? '−' : '+'}</span>
              </div>
              {open && <div className="acc-panel">{renderTabContent(t.id)}</div>}
            </div>
          )
        })}
      </div>
  ...
  /* ── VERSION DESKTOP — menu lateral + panneau ── */
  return (
    <div style={{maxWidth:900}}>
      {SavedToast}
      <div style={{display:'flex',gap:24}}>
        ...
        <div style={{flex:1,minWidth:0}}>{renderTabContent(tab)}</div>
      </div>
    </div>
  )
}
