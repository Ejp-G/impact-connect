...
export default function SuiviClient({ tasks: initialTasks, profile, profiles = [] }) {
  const [tasks, setTasks] = useState(initialTasks)
  const [selectedTaskId, setSelectedTaskId] = useState(null)
  const router = useRouter()

  async function toggleTask(e, id) {
    e.stopPropagation()
  ...
  function TaskGroup({ title, items, co }) {
    ...
    return (
      <div style={{ marginBottom: 24 }}>
        ...
        {items.map(t => (
          <div key={t.id} onClick={() => setSelectedTaskId(t.id)} style={{ background: '#fff', borderRadius: 12, padding: '14px 16px', borderLeft: `4px solid ${co}`, boxShadow: '0 1px 4px rgba(0,0,0,.04)', display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8, cursor: 'pointer' }}>
            <div onClick={e => toggleTask(e, t.id)} style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${co}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: '#fff' }}>
