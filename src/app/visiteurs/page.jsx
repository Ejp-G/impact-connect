...
export default async function VisiteursPage({ searchParams }) {
  ...
  let query = supabase.from('contacts')
    .select(`id,first_name,last_name,sex,phone,email,commune,quartier,stage,integration_score,alert_level,is_minor,created_at,first_visit_date,contact_preference,
  ...
  // 'Aujourd'hui' se base desormais sur la vraie date de premiere
  // visite (first_visit_date), pas sur created_at (date d'enregistrement
  // en base, qui differe en cas de saisie tardive ou d'import).
  const todayStr = new Date().toISOString().split('T')[0]
  const stats = {
    total: contacts?.length || 0,
    alerts: contacts?.filter(c => c.alert_level === 'red').length || 0,
    today: contacts?.filter(c => c.first_visit_date === todayStr).length || 0,
