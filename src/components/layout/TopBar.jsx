  ...
  return (
    <div className="tb">
      ...
      <div style={{position:'relative'}}>
        ...
        {open && (
          <div style={{position:'absolute',right:0,top:48,width:320,background:'#fff',borderRadius:16,boxShadow:'0 8px 32px rgba(0,0,0,.12)',border:'1px solid #F1F5F9',zIndex:1000}}>
            <div style={{padding:'14px 16px',borderBottom:'1px solid #F1F5F9',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:14,fontWeight:700}}>Notifications</span>
              <span style={{fontSize:11,color:'var(--re)',fontWeight:600}}>{notifs.length} non lues</span>
            </div>
            {notifs.length === 0 && <div style={{padding:24,textAlign:'center',color:'var(--gy)',fontSize:13}}>✅ Tout est à jour !</div>}
            {notifs.map(n => (
