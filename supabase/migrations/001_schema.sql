-- ═══════════════════════════════════════════════════════════════════
-- IMPACT CONNECT — SCHÉMA BASE DE DONNÉES SUPABASE v1.0
-- ═══════════════════════════════════════════════════════════════════
...
INSERT INTO communes (name, sector) VALUES
  ('Pointe-a-Pitre','Centre'),('Abymes','Centre'),
  ('Baie-Mahault','Nord-Est'),('Le Gosier','Sud'),
  ('Sainte-Anne','Sud'),('Saint-Francois','Sud-Est'),
  ('Capesterre','Basse-Terre'),('Basse-Terre','Basse-Terre'),
  ('Lamentin','Nord-Est'),('Morne-a-l Eau','Nord');
...
CREATE TABLE contacts (
  ...
  is_minor BOOLEAN GENERATED ALWAYS AS (
    date_of_birth IS NOT NULL AND DATE_PART('year',AGE(date_of_birth)) < 18
  ) STORED,
  phone TEXT, whatsapp TEXT, email TEXT,
  commune TEXT, commune_id UUID REFERENCES communes(id) ON DELETE SET NULL,
  quartier TEXT,
  first_visit_date DATE DEFAULT CURRENT_DATE,
  first_visit BOOLEAN DEFAULT true,
  salvation_call BOOLEAN DEFAULT false,
  how_found TEXT, situation TEXT,
  wants_contact BOOLEAN DEFAULT true,
  wants_fi BOOLEAN DEFAULT true,
  interests TEXT[], prayer_request TEXT,
  ...
  parent_phone TEXT, parent_email TEXT, parent_relation TEXT,
  ...
  first_fi_date DATE, second_fi_date DATE,
  baptism_date DATE, archived_at TIMESTAMPTZ
);
...
CREATE TABLE fi_attendance (
  ...
  contact_id UUID REFERENCES contacts(id) ON DELETE CASCADE NOT NULL,
  date DATE NOT NULL, present BOOLEAN DEFAULT true,
...
CREATE TABLE communication_templates (
  ...
  subject TEXT, content TEXT NOT NULL, variables TEXT[],
  trigger TEXT, active BOOLEAN DEFAULT true,
  created_by UUID REFERENCES profiles(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
INSERT INTO communication_templates (name,channel,content,variables,trigger) VALUES
  ('Bienvenue','whatsapp','Bonjour {prenom}, nous sommes ravis de vous avoir parmi nous ! Nous vous contacterons bientôt.',ARRAY['{prenom}'],'bienvenue'),
...
CREATE TABLE communication_logs (
  ...
  channel TEXT NOT NULL, direction TEXT DEFAULT 'outbound',
...
DECLARE v_c contacts%ROWTYPE; v_s INTEGER := 0; v_p INTEGER; v_t INTEGER;
BEGIN
  ...
  v_s := CASE v_c.stage
    ...
    WHEN 'leader_pot' THEN 92 WHEN 'leader' THEN 100 ELSE 0 END;
...
BEGIN
  ...
  LEFT JOIN (SELECT assigned_to, COUNT(*) c FROM contacts WHERE status='active' GROUP BY assigned_to) L
    ON L.assigned_to = p.id
  WHERE p.role IN ('equipe_suivi','responsable_suivi') AND p.sex=p_sex AND p.active=true
  ORDER BY COALESCE(L.c,0) ASC LIMIT 1;
  IF v_agent IS NOT NULL THEN
    UPDATE contacts SET assigned_to=v_agent, assignment_date=NOW() WHERE id=p_contact;
    INSERT INTO audit_log(action,entity_type,entity_id,details)
    VALUES('Attribution agent auto','contact',p_contact,jsonb_build_object('agent',v_agent,'genre',p_sex));
  END IF;
  RETURN v_agent;
END; $$ LANGUAGE plpgsql SECURITY DEFINER;
...
DECLARE v_fi UUID;
BEGIN
  SELECT cfm.fi_id INTO v_fi FROM commune_fi_mapping cfm
  JOIN familles_impact fi ON fi.id=cfm.fi_id
  WHERE cfm.commune_id=p_commune AND fi.status='active'
    AND (SELECT COUNT(*) FROM contacts WHERE fi_id=fi.id AND status='active') < fi.capacity
  ORDER BY cfm.priority LIMIT 1;
  IF v_fi IS NOT NULL THEN
    ...
    FROM familles_impact fi WHERE fi.id=v_fi AND fi.pilot_id IS NOT NULL;
  END IF;
  RETURN v_fi;
END; $$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION create_follow_up_tasks(p_contact UUID) RETURNS VOID AS $$
DECLARE v_c contacts%ROWTYPE; v_cfg JSONB;
BEGIN
  ...
  IF (v_cfg->>'j7')::BOOLEAN THEN
    INSERT INTO tasks(contact_id,assigned_to,type,title,priority,due_date,auto_created)
    VALUES(p_contact,v_c.assigned_to,'relance','Relance J+7','normal',v_c.created_at::DATE+7,true); END IF;
  IF (v_cfg->>'j14')::BOOLEAN THEN
    INSERT INTO tasks(contact_id,assigned_to,type,title,priority,due_date,auto_created)
    VALUES(p_contact,v_c.assigned_to,'relance','Relance J+14','normal',v_c.created_at::DATE+14,true); END IF;
  IF (v_cfg->>'j21')::BOOLEAN THEN
    INSERT INTO tasks(contact_id,assigned_to,type,title,priority,due_date,auto_created)
    VALUES(p_contact,v_c.assigned_to,'relance','Relance J+21','high',v_c.created_at::DATE+21,true); END IF;
  IF (v_cfg->>'j30')::BOOLEAN THEN
    INSERT INTO tasks(contact_id,assigned_to,type,title,priority,due_date,auto_created)
    VALUES(p_contact,v_c.assigned_to,'relance','Relance J+30 - Urgent','urgent',v_c.created_at::DATE+30,true); END IF;
END; $$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  IF NEW.is_minor AND NEW.parental_status='not_required' THEN
    UPDATE contacts SET parental_status='pending' WHERE id=NEW.id;
  END IF;
  INSERT INTO audit_log(action,entity_type,entity_id,details)
  VALUES('Creation visiteur','contact',NEW.id,jsonb_build_object('name',NEW.first_name||' '||NEW.last_name,'commune',NEW.commune));
  RETURN NEW;
END; $$ LANGUAGE plpgsql SECURITY DEFINER;
CREATE TRIGGER on_contact_created AFTER INSERT ON contacts FOR EACH ROW EXECUTE FUNCTION trigger_new_contact();
...
BEGIN
  IF NEW.stage <> OLD.stage THEN
    ...
    INSERT INTO audit_log(action,entity_type,entity_id,details)
    VALUES('Changement stage','contact',NEW.id,jsonb_build_object('from',OLD.stage,'to',NEW.stage));
  END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql;
CREATE TRIGGER on_stage_change BEFORE UPDATE OF stage ON contacts FOR EACH ROW EXECUTE FUNCTION trigger_stage_change();

CREATE OR REPLACE FUNCTION update_alerts_and_scores() RETURNS VOID AS $$
BEGIN
  UPDATE contacts SET alert_level='red' WHERE status='active' AND stop_relances=false
    AND ((last_contact_at IS NULL AND created_at < NOW()-INTERVAL '3 days')
    OR last_contact_at < NOW()-INTERVAL '30 days') AND stage IN ('visiteur','contacte','invite_fi');
  UPDATE contacts SET alert_level='orange' WHERE status='active' AND stop_relances=false
    AND last_contact_at BETWEEN NOW()-INTERVAL '29 days' AND NOW()-INTERVAL '14 days' AND alert_level!='red';
  UPDATE contacts SET alert_level='green' WHERE status='active'
    AND (last_contact_at > NOW()-INTERVAL '14 days' OR stage IN ('integre','parcours','bapteme','service','leader_pot','leader'))
...
CREATE OR REPLACE FUNCTION my_role() RETURNS TEXT AS $$ SELECT role FROM profiles WHERE id=auth.uid(); $$ LANGUAGE sql STABLE SECURITY DEFINER;
CREATE OR REPLACE FUNCTION my_fi() RETURNS UUID AS $$ SELECT fi_id FROM profiles WHERE id=auth.uid(); $$ LANGUAGE sql STABLE SECURITY DEFINER;

-- PROFILES
CREATE POLICY "own_profile" ON profiles FOR SELECT USING (id=auth.uid() OR my_role() IN ('admin','superviseur','responsable_integration','responsable_suivi'));
...
-- CONTACTS
CREATE POLICY "admin_contacts" ON contacts FOR ALL USING (my_role() IN ('admin','responsable_integration','responsable_suivi'));
CREATE POLICY "integration_insert" ON contacts FOR INSERT WITH CHECK (my_role() IN ('equipe_integration','responsable_integration','admin'));
CREATE POLICY "suivi_read" ON contacts FOR SELECT USING (assigned_to=auth.uid() OR my_role() IN ('admin','responsable_integration','responsable_suivi','superviseur'));
CREATE POLICY "suivi_update" ON contacts FOR UPDATE USING (assigned_to=auth.uid() OR my_role() IN ('admin','responsable_integration','responsable_suivi'));
CREATE POLICY "pilote_read" ON contacts FOR SELECT USING (fi_id=my_fi() OR my_role() IN ('admin','superviseur','responsable_integration'));

-- TASKS
CREATE POLICY "own_tasks" ON tasks FOR SELECT USING (assigned_to=auth.uid() OR my_role() IN ('admin','responsable_suivi','responsable_integration'));
CREATE POLICY "admin_tasks" ON tasks FOR ALL USING (my_role() IN ('admin','responsable_suivi','responsable_integration'));
...
CREATE POLICY "pilote_update_fi" ON familles_impact FOR UPDATE USING (pilot_id=auth.uid() OR copilot_id=auth.uid());
...
-- AUDIT
CREATE POLICY "read_audit" ON audit_log FOR SELECT USING (my_role() IN ('admin','responsable_integration','responsable_suivi'));
